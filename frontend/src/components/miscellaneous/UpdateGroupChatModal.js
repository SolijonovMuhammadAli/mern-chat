...
const UpdateGroupChatModal = ({ fetchMessages, fetchAgain, setFetchAgain }) => {
  ...
  const handleSearch = async (query) => {
    ...
    try {
      ...
      const { data } = await axios.get(`https://lazy-pear-sea-lion-tam.cyclic.app/api/user?search=${search}`, config);
      console.log(data);
      setLoading(false);
  ...
  return (
    <>
      <IconButton d={{ base: "flex" }} icon={<ViewIcon />} onClick={onOpen} style={{ display: "flex" }} />
      ...
      <Modal onClose={onClose} isOpen={isOpen} isCentered>
        ...
        <ModalContent>
          ...
          <ModalBody d="flex" flexDir="column" alignItems="center" style={{ display: "flex" }}>
            <Box w="100%" d="flex" flexWrap="wrap" pb={3} style={{ display: "flex" }}>
